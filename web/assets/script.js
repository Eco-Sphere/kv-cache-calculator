const MODELS = [
  {family:"MiniMax",id:"minimax-m3",label:"MiniMax M3",type:"minimax_msa",layers:60,fullLayers:3,sparseLayers:57,heads:4,dim:128,indexDim:128,indexQHeads:4,indexKHeads:1,indexBlockSize:128,indexTopkBlocks:16,indexLocalBlocks:1,mtpModules:7,nextNLayers:1,max:1048576,fixedIndexerPrecision:2,source:"MiniMaxAI/MiniMax-M3"}
];
...
function calculate(){const m=selected();const tokens=Math.max(1,+$("tokens").value||1);const seq=Math.max(1,+$("sequences").value||1);const tp=Math.max(1,Math.floor(+$("tensor-parallel").value||1));const p=+$("precision").value;let kvPerToken,indexerPerToken=0,label,note,rows;
  ...
  if(m.type==="minimax_msa")renderBreakdown([
    ...
    ["Model fields","num_hidden_layers=60, full_attention_layers=3, sparse_attention_layers=57, num_key_value_heads=4, head_dim=128, index_head_dim=128, index_q_heads=4, index_k_heads=1, index_block_size=128, index_topk_blocks=16, index_local_blocks=1, indexer_fixed_precision_id=bf16_fp16"],
