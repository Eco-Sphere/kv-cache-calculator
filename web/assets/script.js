...
function calculate(){const m=selected();const tokens=Math.max(1,+$("tokens").value||1);const seq=Math.max(1,+$("sequences").value||1);const p=+$("precision").value;let kvPerToken,indexerPerToken=0,label,note,rows;
  ...
  const perToken=kvPerToken+indexerPerToken,one=perToken*tokens,kvTotal=kvPerToken*tokens*seq,indexerTotal=indexerPerToken*tokens*seq,total=one*seq;$("total-gib").textContent=`${(total/1024**3).toFixed(5)} GiB`;$("total-gb").textContent=`= ${(total/1e9).toFixed(5)} GB`;
