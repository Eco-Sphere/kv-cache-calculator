const MODELS = [
  {family:"MiniMax",id:"minimax-m3",label:"MiniMax M3",type:"minimax_msa",layers:60,fullLayers:3,sparseLayers:57,heads:4,dim:128,indexDim:128,indexQHeads:4,indexKHeads:1,indexBlockSize:128,indexTopkBlocks:16,indexLocalBlocks:1,mtpModules:7,nextNLayers:1,max:1048576,source:"MiniMaxAI/MiniMax-M3"}
];
...
function syncTpOptions(m){const select=$("tensor-parallel");const previous=Number(select.value);const options=Array.from({length:m.heads},(_,i)=>i+1).filter(tp=>m.heads%tp===0);select.replaceChildren(...options.map(tp=>new Option(String(tp),String(tp))));select.value=options.includes(previous)?String(previous):String(options[0])}
function syncModel(){const m=selected();$("tokens").value=Math.min(1024,m.max);syncTpOptions(m);const hasIndexer=m.type==="mla"||m.type==="minimax_msa";$("indexer-control").hidden=!hasIndexer;$("indexer").disabled=false;$("draft-control").hidden=!m.draft;$("draft").checked=false;calculate()}
...
function calculate(){const m=selected();const tokens=Math.max(1,+$("tokens").value||1);const seq=Math.max(1,+$("sequences").value||1);const tp=Math.max(1,Math.floor(+$("tensor-parallel").value||1));const p=+$("precision").value;let kvPerToken,indexerPerToken=0,label,note,rows;
  ...
  const perToken=kvPerToken+indexerPerToken,one=perToken*tokens,kvTotal=kvPerToken*tokens*seq,indexerTotal=indexerPerToken*tokens*seq,total=one*seq,perDevice=kvTotal/tp+indexerTotal;$("per-device-gib").textContent=`${(perDevice/1024**3).toFixed(5)} GiB`;$("per-device-gb").textContent=`= ${(perDevice/1e9).toFixed(5)} GB`;$("total-cache-metric").textContent=formatBytes(total);
  ...
  if(m.type==="minimax_msa")renderBreakdown([
    ...
    ["Index Q heads",m.indexQHeads,"Indexer query heads used for scoring selected KV blocks."],
    ["Index K heads",m.indexKHeads,"The Indexer stores one key head, replicated across tensor-parallel ranks."],
    ...
    ["Model fields","num_hidden_layers=60, full_attention_layers=3, sparse_attention_layers=57, num_key_value_heads=4, head_dim=128, index_head_dim=128, index_q_heads=4, index_k_heads=1, index_block_size=128, index_topk_blocks=16, index_local_blocks=1"],
    ["KV precision bytes",p],
    ["Indexer precision bytes",+$("indexer").value],
    ["Tensor parallel size",tp],
    ["Per-device cache size",formatBytes(perDevice)]
